...
return (
    ...
    >
      <button className="bg-zinc-800 hover:bg-blue-700 text-white shadow-md mt-4 ml-4" onClick={() => router.push("/")}> &larr; Back to Home</button>
        ...
        {selectedVideo && (
          <Dialog open={!!selectedVideo} onOpenChange={() => setSelectedVideo(null)}>
            <DialogContent className="max-w-4xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl">
              <DialogHeader>
                <DialogTitle className="text-slate-800 dark:text-slate-100">{selectedVideo.title || "Video"}</DialogTitle>
              </DialogHeader>
              <div className="aspect-video bg-black rounded-md overflow-hidden">
                <div
                  className="w-full h-full"
                  dangerouslySetInnerHTML={{ __html: selectedVideo.youtubeEmbedCode || "" }}
                />
              </div>
              {selectedVideo.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-4">
                  {selectedVideo.tags.map((tag) => (
                    <Badge key={tag} variant="outline" className="bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-600">
                      {tag}
                    </Badge>
                  ))}
                </div>
